...
const OtherIncomePage = () => {
  ...
  const [fields, setFields] = useState<IncomeField[]>([{ label: "", amount: "" }]);
  const [filterDate, setFilterDate] = useState<string>("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [entries, setEntries] = useState<OtherIncomeRecord[]>([]);
  ...
  const fetchEntries = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("id, income_date, label, amount, created_at")
      ...
      toast({
        ...
        description: error.message ?? "Unexpected error while loading income records.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleFieldChange = (index: number, key: keyof IncomeField, value: string) => {
    setFields((prev) => {
      const next = [...prev];
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((entry) => entry.income_date === filterDate);
  }, [entries, filterDate]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!profile?.id) {
      toast({
        ...
        description: "You need to be signed in to record income entries.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (!incomeDate) {
      toast({
        title: "Missing date",
        description: "Please select the income date.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (sanitizedFields.length === 0) {
      toast({
        ...
        description: "Add at least one income field with a description and amount.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (invalidField) {
      toast({
        title: "Invalid entry",
        description: "Each income entry needs a label and a non-negative amount.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const { error } = await supabase.from("other_income_entries").insert(payload);
      ...
      toast({
        ...
        description: payload.length === 1
          ? "The income entry has been saved."
          : `${payload.length} income entries have been saved.`,
      });
      ...
      toast({
        ...
        description: error.message ?? "Unexpected error while saving income entries.",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 px-4 py-4 md:py-6">
      <div className="space-y-1">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-slate-900">Other Income</h1>
</div>
            ...
            <div className="space-y-4">
              {fields.map((field, index) => (
                ...
                >
                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="space-y-2">
                      <Label htmlFor={`income-label-${index}`}>Field label</Label>
                      <Input
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`income-amount-${index}`}>Amount (LKR)</Label>
                      <Input
      ...
      <Card className="border-slate-200">
        <CardHeader className="bg-slate-50 border-b border-slate-200">
          <CardTitle className="text-lg font-semibold text-slate-900">Recent income entries</CardTitle>
                ...
                {!isLoading && filteredEntries.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={3} className="py-8 text-center text-slate-500">
                      No income records found for the selected range.
                    </TableCell>
                  </TableRow>
                )}
                {isLoading && (
